...
if __name__ == '__main__':
    ...
    mlu_iamge_style = image_style.to('mlu')
    net = VGG19().cpu()
    g_net = TransNet().cpu()
    ...
    optimizer = torch.optim.Adam(mlu_g_net.parameters(), lr=0.01)
    ...
    s1, s2, s3, s4 = mlu_net(mlu_iamge_style)
    ...
    while j <= epochs:
        for i, image in enumerate(data_loader):
            ...
            loss_ls1 = mlu_loss_func(get_gram_matrix(out1), get_gram_matrix(s1))
            loss_ls2 = mlu_loss_func(get_gram_matrix(out2), get_gram_matrix(s2))
            loss_ls3 = mlu_loss_func(get_gram_matrix(out3), get_gram_matrix(s3))
            loss_ls4 = mlu_loss_func(get_gram_matrix(out4), get_gram_matrix(s4))
            # TODO：loss_s1-loss_s4相加得到风格损失loss_s
            loss_s = loss_ls1 + loss_ls2 + loss_ls3 + loss_ls4

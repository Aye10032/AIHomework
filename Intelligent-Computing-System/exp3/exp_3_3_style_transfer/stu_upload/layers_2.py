    ...
    for idxh in range(_height):
        for idxw in range(_width):
            out[:, :, :, idxh * _width + idxw] = \
                input[:, :, idxh * stride:idxh * stride + kernel_size,
                idxw * stride:idxw * stride + kernel_size].reshape(N, C, -1)
    return out
...
class ConvolutionalLayer(object):
    ...
    def backward_speedup(self, top_diff):
        ...
        # col2img. [[N, C, H, W]]
        bottom_diff = col2img(col_bottom_diff, height_pad, width_pad, self.kernel_size, self.channel_in, self.padding,
                              self.stride)
...
class MaxPoolingLayer(object):
    ...
    def forward_speedup(self, input):
        ...
        self.col_input = img2col(self.input, self.height_out, self.width_out, self.kernel_size, self.stride).reshape(
            self.N, self.C, -1, self.height_out, self.width_out)
        # [N, C, 1, h_out, w_out]
